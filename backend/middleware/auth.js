...
async function verifyToken(req, res, next) {
  ...
  jwt.verify(bearerToken, process.env.JWT_SECRET, async (err, decoded) => {
    ...
    try {
      // Verificar que el usuario siga activo, con acceso válido y token_version coincida
      const result = await db.query(
        'SELECT activo, fin_acceso, token_version FROM users WHERE id = $1',
        [decoded.id]
      );
      ...
      // Validar token_version para invalidar sesiones tras cambio de contraseña
