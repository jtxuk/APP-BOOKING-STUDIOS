...
router.post('/users/:id/reset-password', async (req, res) => {
  try {
    ...
    // Actualizar: contraseña, marcar must_change_password=true, incrementar token_version
    const result = await db.query(
      `UPDATE users 
       SET password_hash = $1, 
       ...
       WHERE id = $2 
       RETURNING id, name, email, must_change_password, token_version`,
      [hashedPassword, id]
    );

    res.json({
      message: 'Contraseña resetrada. El usuario debe cambiarla en el próximo login.',
      user: result.rows[0]
    });
  } catch (error) {
...
// GET /api/admin/bookings - Ver todas las reservas confirmadas
...
// GET /api/admin/bookings/history - Histórico completo de reservas (confirmadas, canceladas, completadas)
router.get('/bookings/history', async (req, res) => {
  try {
    ...
    // Filtro por status si se proporciona (confirmed, cancelled, etc.)
    if (status) {
